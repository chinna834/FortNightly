...
typealias Completion<T: RequestObject> = (_ success: Bool,_ response: T.response?,_ error: FNError?) -> Void
...
class NetworkManager: NSObject {
    ...
    final func send<T:RequestObject>(r:T,completion:@escaping Completion<T>) {
        
        guard let request = createURLRequest(r: r) else {
            completion(false, nil, FNError(description: Constants.errorInRequest, responseCode: 0, error: nil))
            return
        }
        ...
        let task = session.dataTask(with: request) { (data, response, error) in
            ...
            guard let r = response as? HTTPURLResponse else{
                completion(false, nil, FNError(description: Constants.commonError, responseCode: 0, error: error))
                return
            }
            
            guard r.statusCode == 200 else {
                completion(false, nil, FNError(description: Constants.commonError, responseCode: r.statusCode, error: error))
                return
            }
    ...
    static func downloadImage(url: URL, completion: @escaping (Result<Data>) -> Void) {
        let session = URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            guard let data = data, error == nil else {
                completion(.failure(FNError(description: Constants.commonError, responseCode: 0, error: error)))
                return
            }
